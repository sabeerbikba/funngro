        ...
        <nav class="border-b border-gray-800 sticky top-0 z-50 bg-[#111e2c]">
            ...
            <div
                ...
                <nav role="navigation"
                    ...
                    <NuxtLink v-for="link in navPages" :key="link" :to="link" :class="[
                        'text-left mx-auto text-white py-2 px-4 block relative align-top max-lg:w-auto max-lg:py-3 max-lg:px-0 max-xs:text-center',
                        link === routePath && '!text-[#47c480]']"
                        :aria-current="link === routePath ? 'page' : undefined">
                        {{ link.replace(/^\//, '').replace(/^\w/, (c) => c.toUpperCase()) }}
                    </NuxtLink>
                    <div class="gap-4 text-center flex max-xs:justify-center max-xs:my-2">
                        <a v-for="(app, index) in downloadAppLinks" :key="index" :href="app.link"
                            class="cursor-pointer text-inherit inline-block max-w-full text-white py-2 px-4 text-xl max-lg:py-3 max-lg:px-0 p-8">
                            <div v-html="app.svgIcon" class="w-6 h-6 max-xs:h-8 max-xs:w-8" />
                        </a>
                    </div>
                    <UiCompanyLoginBtn />
                </nav>
